...
public class NormalPodzolSurfaceBuilder extends SurfaceBuilder<SurfaceBuilderConfig> {
    ...
    public void buildSurface(Random random, IChunk chunkIn, Biome biomeIn, int x, int z, int startHeight, double noise, BlockState defaultBlock, BlockState defaultFluid, int seaLevel, long seed, SurfaceBuilderConfig config) {
        SurfaceBuilderConfig podzolConfig = new SurfaceBuilderConfig(WN.getBlockByID("minecraft:podzol").getDefaultState(), config.getUnderWaterMaterial(), config.getUnderWaterMaterial());
