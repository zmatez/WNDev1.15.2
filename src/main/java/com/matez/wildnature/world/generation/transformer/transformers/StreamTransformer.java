...
import com.matez.wildnature.world.generation.heightmap.modules.StreamGenerator;
import com.matez.wildnature.world.generation.layer.grid.GridBiomeLayer;
import com.matez.wildnature.world.generation.transformer.BiomeTransformer;
import net.minecraft.world.biome.Biome;

public class StreamTransformer extends BiomeTransformer {
    ...
    protected Biome apply(Biome oldBiome, TempCategory tempCategory, WetCategory wetCategory, Cell cell, MainBiomeTransformer.TerrainCategory category, float identity) {
        if(!isOcean(oldBiome)){
            if(StreamGenerator.isStream(cell) && !RiverGenerator.isRiver(cell)){
                //return WNBiomes.STREAM_LOWLAND;
            }
        }
        return oldBiome;
    }
