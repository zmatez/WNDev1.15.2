...
public class WNBiomeFeatures extends DefaultBiomeFeatures {
    ...
    public static void addNewStoneVariants(Biome biomeIn) {
        int seaLevel = 40;
        int worldHeight = 255;
        ArrayList<Block> rocks = new ArrayList<>(Arrays.asList(WN.RegistryEvents.rockRegistry.rocks));
        for(int i = 0; i < rocks.size(); i++){
            Block b = WN.getBlockByID(rocks.get(i).getRegistryName().toString());
            RockBase r = null;
            ...
            if(CommonConfig.generateRocks.get()){
                ...
                if(!CommonConfig.getQuartzite.get() && r.getRegistryName().getPath().equals("quartzite")){
                    gen = false;
                }
                if(!CommonConfig.genBlueSlate.get() && r.getRegistryName().getPath().equals("slate_blue")){
                    gen = false;
                }
                if(!CommonConfig.genPurpleSlate.get() && r.getRegistryName().getPath().equals("slate_purple")){
                    gen = false;
                }
                if(gen) {
    ...
    public static void addNewOres(Biome biomeIn) {
        ...
        if(biomeIn.getTempCategory()== Biome.TempCategory.COLD){
            ...
            biomeIn.addFeature(GenerationStage.Decoration.UNDERGROUND_ORES, WNFeatures.GEM_FORMATION_FEATURE.withConfiguration(new BlockVeinFeatureConfig(WNBlocks.SAPPHIRE_FORMATION.getDefaultState().with(FormationBase.TYPE, FormationType.SMALL), 1,1)).withPlacement(Placement.COUNT_RANGE.configure(new CountRangeConfig(CommonConfig.sapphireBigRarity.get(), 0, 0, 30))));
        }else {
            ...
            biomeIn.addFeature(GenerationStage.Decoration.UNDERGROUND_ORES, WNFeatures.GEM_FORMATION_FEATURE.withConfiguration(new BlockVeinFeatureConfig(WNBlocks.RUBY_FORMATION.getDefaultState().with(FormationBase.TYPE, FormationType.SMALL), 1,1)).withPlacement(Placement.COUNT_RANGE.configure(new CountRangeConfig(CommonConfig.rubyBigRarity.get(), 0, 0, 30))));
        }
        biomeIn.addFeature(GenerationStage.Decoration.UNDERGROUND_ORES, WNFeatures.GEM_FORMATION_FEATURE.withConfiguration(new BlockVeinFeatureConfig(WNBlocks.AMETHYST_FORMATION.getDefaultState().with(FormationBase.TYPE, FormationType.SMALL), 1,1)).withPlacement(Placement.COUNT_RANGE.configure(new CountRangeConfig(CommonConfig.amethystSmallRarity.get(), 0, 0, 14))));
        biomeIn.addFeature(GenerationStage.Decoration.UNDERGROUND_ORES, WNFeatures.GEM_FORMATION_FEATURE.withConfiguration(new BlockVeinFeatureConfig(WNBlocks.AMETHYST_FORMATION.getDefaultState().with(FormationBase.TYPE, FormationType.BIG), 1,1)).withPlacement(Placement.COUNT_RANGE.configure(new CountRangeConfig(CommonConfig.amethystBigRarity.get(), 0, 0, 10))));
        ...
        if(BiomeDictionary.getTypes(biomeIn).contains(BiomeDictionary.Type.RIVER) || BiomeDictionary.getTypes(biomeIn).contains(BiomeDictionary.Type.OCEAN)){
            biomeIn.addFeature(GenerationStage.Decoration.UNDERGROUND_ORES, WNFeatures.GEM_FORMATION_FEATURE.withConfiguration(new BlockVeinFeatureConfig(WNBlocks.MALACHITE_FORMATION.getDefaultState().with(FormationBase.TYPE, FormationType.SMALL), 1,1)).withPlacement(Placement.COUNT_RANGE.configure(new CountRangeConfig(CommonConfig.malachiteRarity.get(), 0, 0, 40))));
