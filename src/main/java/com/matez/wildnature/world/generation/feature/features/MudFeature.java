...
public class MudFeature extends Feature<CountConfig> {
   public MudFeature(Function<Dynamic<?>, ? extends CountConfig> p_i51446_1_) {
      super(p_i51446_1_);
      setRegistryName("wildnature","mud_feature");
   }
                  ...
                  for(int h = -height; h < height; h++){
                     for(int d = 0; d < depth; d++){
                        ...
                        if(worldIn.getBlockState(p).isSolid()) {
                           worldIn.setBlockState(p, WNBlocks.MUD.getDefaultState(), 2);
                        }
                     }
                  }
               }
