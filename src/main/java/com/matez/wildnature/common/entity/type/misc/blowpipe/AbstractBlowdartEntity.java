...
public abstract class AbstractBlowdartEntity extends Entity implements IProjectile {
    protected static final DataParameter<Byte> CRITICAL = EntityDataManager.createKey(AbstractBlowdartEntity.class, DataSerializers.BYTE);
    protected static final DataParameter<Optional<UUID>> field_212362_a = EntityDataManager.createKey(AbstractBlowdartEntity.class, DataSerializers.OPTIONAL_UNIQUE_ID);
    protected static final DataParameter<Byte> PIERCE_LEVEL = EntityDataManager.createKey(AbstractBlowdartEntity.class, DataSerializers.BYTE);
    @Nullable
    protected BlockState inBlockState;
    protected boolean inGround;
    protected int timeInGround;
    public AbstractBlowdartEntity.PickupStatus pickupStatus = AbstractBlowdartEntity.PickupStatus.DISALLOWED;
    ...
    private int maxTicksInGround = 1200;
    private Item item = Items.AIR;

    protected AbstractBlowdartEntity(EntityType<? extends AbstractBlowdartEntity> type, World worldIn) {
        super(type, worldIn);
    }

    protected AbstractBlowdartEntity(EntityType<? extends AbstractBlowdartEntity> type, double x, double y, double z, World worldIn) {
        this(type, worldIn);
        this.setPosition(x, y, z);
    }

    protected AbstractBlowdartEntity(EntityType<? extends AbstractBlowdartEntity> type, LivingEntity shooter, World worldIn) {
        this(type, shooter.getPosX(), shooter.getPosYEye() - (double)0.1F, shooter.getPosZ(), worldIn);
        this.setShooter(shooter);
        if (shooter instanceof PlayerEntity) {
            this.pickupStatus = AbstractBlowdartEntity.PickupStatus.ALLOWED;
        }
    ...
    public void setArrowShake(int arrowShake) {
        this.arrowShake = arrowShake;
    }

    public void setMaxTicksInGround(int maxTicksInGround) {
        this.maxTicksInGround = maxTicksInGround;
    }
    ...
    public void setItem(Item item) {
        WN.LOGGER.debug(".Setting item to " + item.getRegistryName());
        this.item = item;
    }
    ...
    protected void onEntityHit(EntityRayTraceResult p_213868_1_) {
        ...
        if (entity1 == null) {
            damagesource = WNDamageSource.causeArrowDamage(this, this);
        } else {
            damagesource = WNDamageSource.causeArrowDamage(this, entity1);
        ...
        if (entity.attackEntityFrom(damagesource, (float)i)) {
            ...
            if (!this.world.isRemote && this.getMotion().lengthSquared() < 1.0E-7D) {
                if (this.pickupStatus == AbstractBlowdartEntity.PickupStatus.ALLOWED) {
    ...
    public void writeAdditional(CompoundNBT compound) {
        ...
        compound.putString("itemName", getItem().getRegistryName().toString());
    }
    ...
    public void readAdditional(CompoundNBT compound) {
        ...
        if (compound.contains("pickup", 99)) {
            this.pickupStatus = AbstractBlowdartEntity.PickupStatus.getByOrdinal(compound.getByte("pickup"));
        } else if (compound.contains("player", 99)) {
            this.pickupStatus = compound.getBoolean("player") ? AbstractBlowdartEntity.PickupStatus.ALLOWED : AbstractBlowdartEntity.PickupStatus.DISALLOWED;
        }
        ...
        if(compound.contains("itemName") && item == Items.AIR){
            Item i = WN.getItemByID(compound.getString("itemName"));
            setItem(i);
        }
    ...
    public void setShooter(@Nullable Entity entityIn) {
        ...
        if (entityIn instanceof PlayerEntity) {
            this.pickupStatus = ((PlayerEntity)entityIn).abilities.isCreativeMode ? AbstractBlowdartEntity.PickupStatus.CREATIVE_ONLY : AbstractBlowdartEntity.PickupStatus.ALLOWED;
        }
    ...
    public void onCollideWithPlayer(PlayerEntity entityIn) {
        if (!this.world.isRemote && (this.inGround || this.getNoClip()) && this.arrowShake <= 0) {
            boolean flag = this.pickupStatus == AbstractBlowdartEntity.PickupStatus.ALLOWED || this.pickupStatus == AbstractBlowdartEntity.PickupStatus.CREATIVE_ONLY && entityIn.abilities.isCreativeMode || this.getNoClip() && this.getShooter().getUniqueID() == entityIn.getUniqueID();
            if (this.pickupStatus == AbstractBlowdartEntity.PickupStatus.ALLOWED && !entityIn.inventory.addItemStackToInventory(this.getArrowStack())) {
                flag = false;
            }
    ...
    public static enum PickupStatus {
        ...
        public static AbstractBlowdartEntity.PickupStatus getByOrdinal(int ordinal) {
